...
def starbound_needs_update(install_dir: str, app_id: str) -> bool:
    if not os.path.isdir(install_dir):
        log.warning(f"Starbound not found in {install_dir}.")
        return True
    try:
...
def workshop_mod_needs_update(workshop_mod_id: str, install_dir: str) -> bool:
    try:
        ...
        if remote_timestamp > local_timestamp:
            ...
            log.info("No new mod update available.")
        return remote_timestamp > local_timestamp
    except (requests.RequestException, KeyError, ValueError, OSError) as e:
        log.error(
